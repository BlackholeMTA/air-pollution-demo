...
PRED_FILE = Path("data/output/predictions_silam_hanoi.csv")

def main():
    df = pd.read_csv(PRED_FILE)

    mae_raw = mean_absolute_error(df["pm25_obs"], df["silam_pm25"])
    rmse_raw = np.sqrt(mean_squared_error(df["pm25_obs"], df["silam_pm25"]))
