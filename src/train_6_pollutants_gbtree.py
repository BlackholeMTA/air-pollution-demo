...
OUT_METRICS = os.path.join(OUTPUT_DIR, "pollutant_metrics_6models_gbtree.csv")
...
def train_one_pollutant(df, pollutant_name, cfg):
    ...
    model_path = os.path.join(OUTPUT_DIR, f"xgb_{pollutant_name}_gbtree_model.pkl")
...
def main():
    ...
    for pollutant_name, cfg in POLLUTANT_CONFIGS.items():
        print("\n" + "=" * 70)
        print(f"ĐANG TRAIN GBTREE: {pollutant_name}")
        print("=" * 70)
